...
const filter1500 = inventors.filter(inventor =>
...
const mapFullNames = inventors.map(inventor =>
...
const sortOrder = inventors.sort((a, b) =>
    a.year > b.year ? 1 : -1
);
...
const reduceTotalYears = inventors.reduce((total, inventor) => {
    return total + (inventor.passed - inventor.year);
}, 0);
console.log(`The total years that inventors lived: ${reduceTotalYears}`);
...
const reduceSumDupli = data.reduce((obj, item) => {
    if(!obj[item]) {
        obj[item] = 0;
    }
    obj[item]++;
